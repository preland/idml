...
node tools/scale-bench/bench.mjs --interactive \
  --target dashboard --viewport laptop-1366x768
```
...
```json
{ "name": "dashboard", "path": "/dashboard", "safeScale": [1.0, 1.3] }
```
...
```json
{ "name": "graph", "path": "/graph", "repeat": 3 }
```
...
```json
{ "name": "graph", "path": "/graph", "repeat": 3, "tolerance": 1 }
```
